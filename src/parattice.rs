...
impl<'a> PaRattice<'a> {
    ...
    fn backward_match(phrase: &Vec<&str>, lattice: &Vec<LatticeNode>, pos: usize, max_depth: usize) -> Vec<(usize, usize)> {
        let mut result = vec![];
            ...
            if phrase_pos == 0 {
                ...
                for &(edge_str, edge_target) in &lattice[lattice_node_id].backwards {
                    if edge_str == phrase[phrase_pos - 1] && lattice[edge_target].depth < max_depth {
                        backward_queue.push_back((phrase_pos - 1, edge_target, cmp::max(depth, lattice[edge_target].depth)));
                    }
                }
            }
        }
    ...
    fn insert_branch(lattice: &mut Vec<LatticeNode<'a>>, state_id_cache: &mut Vec<BTreeSet<usize>>, phrase: &Vec<&'a str>, start_node_id: usize, end_node_id: usize, depth: usize) -> usize {
        ...
        match phrase.len() {
            ...
            2 => {
                ...
                lattice.push(LatticeNode::new((phrase[1].clone(), end_node_id), (phrase[0].clone(), start_node_id), depth));
            ...
            3 => {
                lattice[start_node_id].insert_forward(&phrase[0], new_node_id);
                lattice.push(LatticeNode::new((phrase[1].clone(), new_node_id + 1), (phrase[0].clone(), start_node_id), depth));
                state_id_cache.push(BTreeSet::new());
                lattice.push(LatticeNode::new((phrase[2].clone(), end_node_id), (phrase[1].clone(), new_node_id), depth));
                state_id_cache.push(BTreeSet::new());
            ...
            _ => {
                ...
                for i in 0..phrase.len() - 3 {
                    lattice.push(LatticeNode::new((phrase[i + 2].clone(), new_node_id + i + 2), (phrase[i + 1].clone(), new_node_id + i), depth));
                    state_id_cache.push(BTreeSet::new());
                }
                lattice.push(LatticeNode::new((phrase[phrase.len() - 1].clone(), end_node_id), (phrase[phrase.len() - 2].clone(), new_node_id + phrase.len() - 3), depth));
                state_id_cache.push(BTreeSet::new());
                lattice[end_node_id].insert_backward(&phrase[phrase.len() - 1], new_node_id + phrase.len() - 2);
                new_node_id
            },
        }
    }
    ...
    /// ```
    pub fn get_lattice(&self, words: &Vec<&'a str>, shrink: bool, max_depth: usize) -> Lattice<'a> {
        ...
        if words.is_empty() {
            ...
            lattice.push(LatticeNode::new((words[0].clone(), 1), None, 0));
            state_id_cache.push(BTreeSet::new());
            for node_id in 1..words.len() {
                lattice.push(LatticeNode::new((words[node_id].clone(), node_id + 1), (words[node_id - 1].clone(), node_id - 1), 0));
                state_id_cache.push(BTreeSet::new());
            }
            lattice.push(LatticeNode::new(None, (words[words.len() - 1].clone(), words.len() - 1), 0));
            state_id_cache.push(BTreeSet::new());
        }
        // search phrases
        let mut queue = VecDeque::new();
        ...
        while let Some((lattice_node_id, pma_state_id)) = queue.pop_front() {
            ...
            for &phrase_id in &self.pma[pma_state_id].matched {
                ...
                for (branch_start, depth) in Self::backward_match(phrase, &lattice, lattice_node_id, max_depth) {
                    ...
                    for &paraphrase_id in &self.dict[*group_id] {
                        ...
                        let inserted_first_node_id = Self::insert_branch(&mut lattice, &mut state_id_cache, paraphrase, branch_start, lattice_node_id, depth + 1);
                        let (state_id_cache_current, state_id_cache_next) = get_two_mut_elems(&mut state_id_cache, branch_start, inserted_first_node_id);
                        for pma_state_id_cached in state_id_cache_current.iter() {
                            let pma_state_id_new = Self::next_pma_state_id(&self.pma, *pma_state_id_cached, &paraphrase[0]);
    ...
    fn main_branch_bw(g: &Vec<LatticeNode>, begin: usize, eos: usize) -> usize {
        let mut b = begin;
        while b > eos {
    ...
    fn main_branch_fw(g: &Vec<LatticeNode>, end: usize, eos: usize) -> usize {
        let mut e = end;
        while e > eos {
        ...
        while !updated_node_bw.is_empty() || !updated_node_fw.is_empty() {
            ...
            for &i in &updated_node_bw {
                if !lattice[i].backwards.is_empty() {
                    backward_map.entry(lattice[i].backwards.clone()).or_insert(vec![]).push(i);
                }
            }
            ...
            for nodes in backward_map.values() {
                if nodes.len() >= 2 {
                    for i in 1..nodes.len() {
                        let backward_tmp = mem::replace(&mut lattice[nodes[i]].backwards, BTreeSet::new());
                        ...
                        let forward_tmp = mem::replace(&mut lattice[nodes[i]].forwards, BTreeSet::new());
                        for (edge_str, next_node_id) in forward_tmp {
                            lattice[next_node_id].backwards.remove(&(edge_str, nodes[i]));
            ...
            for &i in &updated_node_fw {
                if !lattice[i].forwards.is_empty() {
                    forward_map.entry(lattice[i].forwards.clone()).or_insert(vec![]).push(i);
                }
            }
    ...
    fn index_left_to_right(lattice: &Vec<LatticeNode<'a>>) -> Vec<LatticeNode<'a>> {
        ...
        for &node_id in &node_id_map_rev {
            ...
            let forward_main = lattice[node_id].forward_main.map(|(x, i)| (x, node_id_map[i]));
            let backward_main = lattice[node_id].backward_main.map(|(x, i)| (x, node_id_map[i]));
            new_lattice.push(
                LatticeNode {
                    forwards: new_forwards,
                    backwards: new_backwards,
                    forward_main: forward_main,
                    backward_main: backward_main,
                    depth: 0,
                });
        }
        new_lattice
