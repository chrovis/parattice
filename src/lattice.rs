...
impl<'a> LatticeNode<'a> {
    pub fn new<T1: Into<Option<(&'a str, usize)>>, T2: Into<Option<(&'a str, usize)>>>(
            forward_main: T1, backward_main: T2, depth: usize) -> LatticeNode<'a> {
...
impl<'a> Lattice<'a> {
        ...
        while offset < data.len() {
            ...
            let forward_main =
                if num_forwards != 0 {
                    ...
                    let forward_main_edge_string = str::from_utf8(
                        &data[offset+8..offset+8+forward_main_num_chars]).unwrap();
                    let forward_main_edge_target = vec_to_usize(
                        &data[offset+8+forward_main_num_chars..offset+16+forward_main_num_chars]);
                    ...
                    for _ in 1..num_forwards {
                        ...
                        let forward_edge_string = str::from_utf8(
                            &data[offset+8..offset+8+forward_num_chars]).unwrap();
            ...
            let backward_main =
                if num_backwards != 0 {
                    ...
                    let backward_main_edge_string = str::from_utf8(
                        &data[offset+8..offset+8+backward_main_num_chars]).unwrap();
                    let backward_main_edge_target = vec_to_usize(
                        &data[offset+8+backward_main_num_chars..offset+16+backward_main_num_chars]);
                    ...
                    for _ in 1..num_backwards {
                        ...
                        let backward_edge_string = str::from_utf8(
                            &data[offset+8..offset+8+backward_num_chars]).unwrap();
    ...
    pub fn dump_dot(&self, is_numbered: bool) -> String {
        ...
        for (i, node) in self.lattice.iter().enumerate() {
            if is_numbered {
                result = result + &format!("\"{}\" [label=\"{}\",shape=plaintext,width=\"0.1\"];\n", i, i);
            } else {
                result = result + &format!("\"{}\" [label=\"\",shape=circle,width=\"0.1\"];\n", i);
            }
            for (j, &edge) in node.forwards.iter().enumerate() {
                result = result + &format!("\"{}-{}-{}\" [label=\"{}\",shape=box];\n", i, j, edge.1, edge.0);
                if edge == node.forward_main.unwrap() {
                    result = result + &format!("\"{}\" -> \"{}-{}-{}\" [arrowhead=none,color=\"#ff0000\"];\n", i, i, j, edge.1);
                } else {
                    result = result + &format!("\"{}\" -> \"{}-{}-{}\" [arrowhead=none];\n", i, i, j, edge.1);
                }
                if self.lattice[edge.1].backward_main == Some((edge.0, i)) {
                    result = result + &format!("\"{}-{}-{}\" -> \"{}\" [color=\"#0000ff\"];\n", i, j, edge.1, edge.1);
                } else {
    ...
    pub fn get_trunk_span(&self, path: Vec<(&'a str, usize)>) -> Vec<(&'a str, usize)> {
        ...
        while !self.trunk.contains_key(&edge_bw.1) {
            let next_edge = self.lattice[edge_bw.1].backward_main.unwrap().clone();
            new_path.push_front((next_edge.0, edge_bw.1));
            edge_bw = next_edge;
        }
        new_path.push_front(("", edge_bw.1));
        let mut edge_fw = new_path.back().unwrap().clone();
        while !self.trunk.contains_key(&edge_fw.1) {
            edge_fw = self.lattice[edge_fw.1].forward_main.unwrap().clone();
